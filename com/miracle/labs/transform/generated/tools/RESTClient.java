...
public class RESTClient {
	 ...
	 @Value("${target.server.url:http://localhost:8080/}")
	 String serverUrl;
	 ...
	 * @throws InvocationTargetException 
	 ...
	 public <T> T executeGET(String applicationpath,String path,  Class<T> returnType,String params) throws InterruptedException, ExecutionException, UnsupportedEncodingException, JsonbException, InstantiationException, IllegalAccessException{
		 ...
		 Client client = new Client(applicationpath+"/"+path.substring(3)+"/"+params,Collections.EMPTY_MAP,GET);
		 ...
		 {	 
			 ...
			 try
			 {
				JsonObject json = JsonConvertor.getJsonObject(data);
				String reply = 	json.get("message").toString();
				t = JsonConvertor.getClassFromJsonString(reply, returnType); 
...
//				 try{
//					//t = setThroughConstructor(returnType,data);
//					 
//					 t = JsonConvertor.getClassFromJsonString(data, returnType);
//				 }
//				 catch ( Exception e1)
//				 {
...
//				 }
	 ...
	 /*private  <T> T setThroughConstructor(Class<T> classT,String data) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException
	 {
		 T t = classT.newInstance();
		 for(Constructor<?> c:classT.getConstructors())
		 {
			 if(c.getParameterCount()==1 && c.getParameterTypes()[0]==String.class)
			 {
				t = classT.cast(c.newInstance(data));
